...
function EmployeePicker({ members, value, onChange }: { members: OrganizationMember[]; value: string; onChange: (id: string) => void }) {
  ...
  const terms = search.trim().toLocaleLowerCase('ru-RU').split(/\s+/).filter(Boolean)
  const matching = members.filter((member) => terms.every((term) => `${member.displayName} ${member.email}`.toLocaleLowerCase('ru-RU').includes(term)))
  return <div className="employee-picker"><span>Сотрудник</span><button className="employee-picker__trigger" type="button" aria-expanded={open} onClick={() => { setOpen((current) => !current); setSearch('') }}><Avatar url={selected?.avatarUrl} name={selected?.displayName ?? '?'} className="employee-picker__avatar" /><span><strong>{selected?.displayName ?? 'Выберите сотрудника'}</strong><small>{selected?.email ?? 'Активный участник организации'}</small></span><svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.7" aria-hidden="true"><path d="m6 8 4 4 4-4" /></svg></button>{open && <div className="employee-picker__menu"><div className="employee-picker__search"><input type="search" aria-label="Найти сотрудника" placeholder="Имя, фамилия или почта" value={search} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => { if (event.key === 'Escape') setOpen(false) }} autoFocus /></div><div className="employee-picker__results">{matching.map((member) => <button type="button" className={member.id === value ? 'active' : ''} onClick={() => { onChange(member.id); setOpen(false); setSearch('') }} key={member.id}><Avatar url={member.avatarUrl} name={member.displayName} className="employee-picker__avatar" /><span><strong>{member.displayName}</strong><small>{member.email}</small></span>{member.id === value && <span className="employee-picker__check">✓</span>}</button>)}{!matching.length && <p className="employee-picker__empty">Сотрудники не найдены</p>}</div></div>}</div>
}
